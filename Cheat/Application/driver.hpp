...
public:
    ...
    bool readHugeMemory(std::uintptr_t moduleBase, void* buffer, size_t size) const noexcept {
        if (moduleBase == 0 || buffer == nullptr || size == 0) {
            return false;
        }

        try {
            for (size_t i = 0; i < size; ++i) {
                // Assuming that the RPM function can handle reading a single byte
                // and that the buffer is a byte array
                reinterpret_cast<std::byte*>(buffer)[i] = RPM<std::byte>(moduleBase + i);
            }
            return true;
        }
